...
plot(g)
B<-matrix(c(0.9,0.46,0.46,0,0.78,0,0,0,0.76),3,3) #transition matrix
PS<-matrix(rep(0,495),45,11)
MS<-matrix(rep(0,495),45,11)
DS<-matrix(rep(0,495),45,11)
#t controls the no. of vertices to be deleted
for (t in 1:3){
  #Population data, patch wise
  ...
  for (w in 1:15){
    ...
    PS[t*w,]<-ps; MS[t*w,]<-ms;DS[t*w,]<-ds  
  }
  
  deg_B <- degree(g) 
  betw_B <- betweenness(g)
  #g delete_vertices or g-c("PM")
  assign(paste("del_",t,sep=""),which(deg_B==max(deg_B))[1])
  g<-(delete_edges(g,incident(g,which(deg_B==max(deg_B))[1])))
